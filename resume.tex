\documentclass[10pt]{article}
\author{August Valera}

\usepackage{comment}
\usepackage{fullpage, hyperref}
\usepackage{xcolor,fontawesome}
\let\orighref\href%
\renewcommand{\href}[2]{\orighref{#1}{#2{\scriptsize~\color{darkgray}\faExternalLink}}}
\renewcommand{\url}[1]{\href{#1}{#1}}
\renewcommand{\familydefault}{\sfdefault}
\usepackage{geometry} % sets size
\geometry{
  letterpaper,
  margin=1in,
  tmargin=1.2in,
  headsep=0.1in,
  headheight=1in,
  footskip=-0.15in
}
\usepackage{titlesec} % disables break after section titles
\titlespacing{\section}{0pt}{-0.7\parskip}{0pt}
\titleformat{\section}
{\normalfont\Large\bfseries}{\thesection}{1em}{}[{\titlerule[0.5pt]}]
\usepackage{parskip} % disables indentation
\usepackage{enumitem}
% disables indentation for lists
\setlist[description]{leftmargin=*,nolistsep}
\setlist[itemize]{leftmargin=*,nolistsep}
\usepackage{multicol}
\setlength{\multicolsep}{0pt}
\usepackage[none]{hyphenat} % disables splitting words
\usepackage{attachfile} % attaches source code
\usepackage{fancyhdr} % headings and footers
\renewcommand{\footrulewidth}{0.4pt}
\pagestyle{fancy}
\fancyhead{}
\fancyhead[L]{\Huge \textbf{August Valera}}
\fancyhead[R]{
  \small
  +1 (616) 284--1018 --- \href{mailto:AugustValera@icloud.com}{AugustValera@icloud.com} \\
  \href{https://linkedin.com/in/AugustValera}{LinkedIn/AugustValera} ---
  \href{https://gitlab.com/4U6U57}{GitLab/4U6U57} ---
  \href{https://github.com/4U6U57}{GitHub/4U6U57}
}
\fancyfoot[L]{
  \small Compiled with \LaTeX\ on\ \textit{\today}
}
\fancyfoot[R]{
  \small Latest at
  \href{https://gitlab.com/4U6U57/resume}{GitLab.com/4U6U57/resume}
  (\href{https://gitlab.com/4U6U57/resume/builds/artifacts/master/file/resume.pdf?job=build}
  {download})
}
\usepackage[yyyymmdd]{datetime} % date for footer in ISO
\renewcommand{\dateseparator}{--}

\begin{document}

\pagenumbering{gobble}

\section*{Education}
\begin{description}
  %\item[High School Graduate] Westmoor High School in Daly City, CA
  \item[Bachelor of Science in Computer Engineering] University of California,
    Santa Cruz, Highest Honors --- \textit{2017}
  \item[Bachelor of Science in Computer Science] University of California, Santa
    Cruz, Honors --- \textit{2017}
  \item[GPA] 3.67, cum laude
\end{description}

\section*{Relevant Coursework}
\begin{itemize}
  \begin{multicols}{2}
    %\item C Programming % CMPE 13/L
    %\item CE Senior Project (XBee, Bluetooth LE) % CMPE 129A/B/C
    %\item Microprocessor System Design (PSoC)% CMPE 121
  \item Computer Networking (bash, Java) % CMPE 150
  \item Network Programming (bash, C, TCP, UDP) % CMPE 156
  \item Abstract Data Types (C, Java)% CMPS 101
  \item Algorithm Analysis (proofs, time complexity)% CMPS 102
    %\item Compiler Design I (C++, Flex, Bison) % CMPS 104A
    %\item Operating Systems (C, FreeBSD) % CMPS 111
  \item Software Engineering (Scrum) % CMPS 115
  \item CS Senior Project (Django, PostgreSQL, Heroku) % CMPS 116/117
  \item Mobile Applications (Android) % CMPS 121
  \item Web Applications (web2py, vue.js) % CMPS 185
  \end{multicols}
\end{itemize}

\section*{Skills \&\& Tools}
\begin{description}
  \begin{multicols}{2}
  \item[General Programming] C, C++, Java, Python
  \item[Shell Scripting] bash, csh, perl
  \item[Web Design] AngularJS, Bootstrap, Django, Firebase,
    Heroku, PostgreSQL, vue.js, web2py
  \item[Mobile Apps] Android (Java), Apache Cordova
  \item[Agile Project Management (scrum)]
  \item[Testing/CI] BATS, GitLab, Travis
  \item[DevOps] git, make, npm, Ubuntu, Vim, VirtualBox
  \end{multicols}
\end{description}

\section*{Projects}
\begin{description}
  \item[\href{https://github.com/grepthink/grepthink}{GrepThink}] Computer
    Science Senior Project --- \textit{Jan 2017--Jun 2017}
    \begin{itemize}
      \item Full Stack Developer for fully functional, educational webapp
        facilitating group formation and management
      \item Worked on database structures, user interface, algorithm for group
        formation, posting feature
      \item Deployed for use with the UCSC Software Engineering class, Summer
        2017, demo: \url{https://grepthink.com}
    \end{itemize}
  \item[\href{https://github.com/HaHaSDP-UCSC}{Help Alert Home Assistance
    Button}] Computer Engineering Senior Design Project --- \textit{Oct 2016--Jun 2017}
    \begin{itemize}
      \item User Experience Developer for senior citizen emergency alert button,
        an open source humanitarian project
      \item Wrote user interface and LCD display logic/drivers for Linux
        prototype and later embedded system (uC)
    \end{itemize}
\end{description}
\begin{comment}
\item[\href{https://github.com/4U6U57/otterhacks17.otterbeactive}{Otter Be
  Active}] Domain.com category winner at Otter Hacks --- \textit{2017}
  \begin{itemize}
    \item Frontend Developer for mobile and web app for locating nearby
      parks/POI
      users to be more active
    \item Apache Cordova and NodeJS for multiplatform application, Google Maps
      Distance Vector API for places
    \item Made in under 36 hours with no prior Cordova experience, demo:
      \url{https://otterbeactive.com}
  \end{itemize}
\item[\href{https://github.com/4U6U57/uberhacks17.convoy}{Convoy}] Third
  Place Winner at UBER Hacks --- \textit{2017}
  \begin{itemize}
    \item Backend Developer for SMS service facilitating group reservation of
      multiple UBER vehicles optimizing cost
    \item Frontend (Twilio) and REST API (ExpressJS and AngularJS),
      incorporating UBER and Google Maps APIs
  \end{itemize}

\item[\href{https://github.com/4U6U57/hackucsc16.autogarden}{Autonomous
  Garden Project}]
  HACK UCSC --- \textit{2016}
  \begin{itemize}
    \item Lead frontend engineer of the web interface for automated
      microcontroller garden monitor
    \item Webapp frontend (AngularJS) and backend (Firebase), server backend
      (Python) uploading data from uC
  \end{itemize}
\item[\href{https://github.com/Fence-UCSC/Storefront}{Storefront}] Fence ---
  \textit{2016}
  \begin{itemize}
    \item UI/UX developer for proof of concept online sales webapp
    \item Worked with web2py web server, frontend (HTML/CSS/JS) and backend
      (Python), SQL database
    \item Agile project management, GitHub collaboration, continuous
      integration, deployment
  \end{itemize}
\end{comment}

\section*{Experience}
\begin{description}
  \item[\href{https://gitlab.com/4U6U57/grading}{Teaching Assistant (Undergraduate)}]
    Computer Science Dept., UC Santa Cruz --- \textit{April 2015--June 2017}
    \begin{itemize}
      \begin{multicols}{3}
      \item Intro CS (Processing)
      \item Java Programming
      \item Data Structures (C/Java)
      \item Abstract Data Types
      \item Algorithm Analysis
      \end{multicols}
    \item Independently lectured lab sections of up to $30$ students
    \item Graded $400+$ students' programming assignments, written homeworks,
      and exams each quarter
    \item Automated grading process with self-designed scripts (csh, Bash, C,
      Python)
  \end{itemize}
\item[\href{https://gitlab.com/4U6U57/hackathons}{Hackathon Participant}] Major
  League Hacking (MLH) --- \textit{2015--2017}
  \begin{itemize}
    \begin{multicols}{2}
    \item
      \href{https://gitlab.com/4U6U57/hackucsc16.autogarden}{HACK UCSC} --- \textit{2016}
    \item \href{https://gitlab.com/4U6U57/hackucsc17.rackin}{HACK UCSC} --- \textit{2017}
    \item \href{https://gitlab.com/4U6U57/uberhacks17.convoy}{UBER Hacks, 3rd
      Place} --- \textit{2017}
    \item \href{https://gitlab.com/4U6U57/sfhacks17.catchthesehands}{SF Hacks,
      2nd Place in Hardware} --- \textit{2017}
    \item
      \href{https://gitlab.com/4U6U57/otterhacks17.otterbeactive}{Otter Hacks,
      Domains.com winner}
      --- \textit{2017}
    \item \href{https://gitlab.com/4U6U57/siliconhacks17.adjustgov}{Silicon
      Hacks, Make School winner} --- \textit{2017}
    \end{multicols}
\end{itemize}
\end{description}

\section*{Awards \&\& Achievements}
\begin{description}
  \item[\href{https://honors.ucsc.edu/honors-programs/deans-honors}{Dean's List}] UC Santa Cruz --- 8 out of 12 quarters attended
  \item[\href{https://ua.soe.ucsc.edu/honors/2016-2017}{Dean's Award}] UC Santa
    Cruz, presented for an outstanding senior project --- \textit{2017}
    %: \\ \textit{Fall '13, Winter '14, Spring '14, Spring '15, Spring '16, Fall '16, Winter '17, Spring '17}
    %\item[Volunteer Service Award] Merrill Student Government --- \textit{2016}
    %\item[Terence Freitas College Service Award] Crown College Graduating Class --- \textit{2017}
\end{description}

\section*{Activities}
\begin{description}
  \item[Student Government] University of California, Santa Cruz
    \begin{itemize}
      \item \textbf{Elected Representative} Student Union Assembly --- \textit{2015--2016}
      \item \textbf{Senator} Crown Student Senate --- \textit{2013--2017}
    \end{itemize}
  \item[Engineering Organizations]~%
    \begin{itemize}
      \item \textbf{Amateur Radio Operator, Technician Class (K6AUG)} UCSC Amateur Radio Club (W6SLG)
        %\item \textbf{Member} Association of Computing Machinery (ACM)
      \item \textbf{Member} Society of Asian Scientists and Engineers (SASE)
    \end{itemize}
  \item[Volunteering]~%
    \begin{itemize}
      \item \textbf{Marketing Communications Chairperson}
        \href{http://SFFilAmLions.org}{San Francisco Fil-Am Lions Club} --- \textit{2015--present}
        %\item \textbf{Attendee Safety Staff (Rovers Dept.)} FanimeCon, San Jose ---
        %  \textit{2013--present}
        %\item \textbf{Associate Member}
        %  Silicon Valley Cyber Lions Club --- \textit{2016--present}
    \end{itemize}

\end{description}
\begin{comment}
\item[Relevant Hobbies] ~
  \begin{itemize}
    \item \textbf{\href{https://coord.info/PR62PFW}{Geocaching (4U6U57)}} 350
      finds
    \item \textbf{Pokemon Go} Team Instinct, Level 21
    \item \textbf{Ingress} Enlightened, Level 5
    \item \textbf{Google Maps Contributor/Local Guide} Level 5, formerly Google
      Maps Maker with 500+ edits
  \end{itemize}
\end{comment}

\end{document}
